...
for trajectory in range(1):
    ...
    for episode in range(episode_max):
        ...
        while not is_terminal_stateP(current_state, n_jobs):
            ...
            picked_action = get_next_actionP(state_index_cur, q_values, epsilon, n_jobs, current_state)
            new_state = current_state
            next_job = Initial_Sol[len(current_state)]
            new_state.insert(picked_action, next_job)  # update the current state
            state_index_new, best_Q_next_state = get_best_Q_next_stateP(str(new_state), q_values_state_table, q_values)
            reward = 1 / objective_value_rewardP(current_state)
            q_values = update_q_valueP(q_values, reward, learning_rate, discount_factor, state_index_cur, picked_action, best_Q_next_state)
    ...
    print(best_sol_obj, round(time.time() - start_time, 2))
