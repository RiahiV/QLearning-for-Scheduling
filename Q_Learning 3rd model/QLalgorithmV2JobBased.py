...
def objective_value(cur_sol, n_mach, process_t):
...
def objective_value_reward(cur_sol, n_jobs, n_mach, process_t):
    how_many_job = len(cur_sol)
...
def last_job_based_rl(n_jobs, n_mach, process_t, episode_limit):
    ...
    for episode in range(episode_limit):
        ...
        while not is_terminal_state(current_state, n_jobs):
            ...
            best_Q_next_state = get_best_Q_next_state(str(new_state), q_values_state_table, q_values)
            reward = 1/objective_value_reward(current_state, n_jobs, n_mach, process_t)
            q_values = update_q_value(q_values, reward, learning_rate, discount_factor, state_index, picked_action, best_Q_next_state)
            current_state = new_state

        current_state_obj = objective_value(current_state, n_mach, process_t)
#        if episode % 500 == 0:
