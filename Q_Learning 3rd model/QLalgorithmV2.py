...
def objective_value_rewardP(cur_sol):
    how_many_job = len(cur_sol)
...
def argmaxP(q_values_input, current_state):
    """
...
def is_terminal_stateP(current_state, n_jobs):
...
def get_next_actionP(current_state_indx, q_values, epsilon, n_jobs, current_state):
    # if a randomly chosen value between 0 and 1 is less than epsilon,
    # then choose the most promising value from the Q-table for this state.

    if np.random.random() < epsilon:
        return argmaxP(q_values[current_state_indx], current_state)
    else:  # choose a random action
        generated_action_rand = random.randint(0, len(current_state))
        return generated_action_rand


def current_state_indexP(current_state, q_values_state_table):
...
def get_best_Q_next_stateP(new_state, q_values_state_table, q_values):
    new_state_index = current_state_indexP(new_state, q_values_state_table)
...
def update_q_values_state_tableP(current_state, q_values_state_table, q_values, n_jobs):
...
def update_q_valueP(q_values_table, reward, learning_rate, discount_factor, state_index, picked_action, best_Q_next_state):
...
from QLalgorithmV2JobBased import *
Initial_Sol = last_job_based_rl(n_jobs, n_mach, process_t, 1000)
episode_max = 5000

initial_Sol_Obj = objective_valueP(Initial_Sol)
# print(" Solution obtained by RL for the initial solution ", Initial_Sol, initial_Sol_Obj)
...
for trajectory in range(5):
    ...
    # using a random solution to create an initial solution
    # Initial_Sol = list(range(0, n_jobs))
    ...
    for episode in range(episode_max):
        ...
        current_state = []
        while not is_terminal_stateP(current_state, n_jobs):
            q_values, q_values_state_table = update_q_values_state_tableP(current_state, q_values_state_table, q_values, n_jobs)
            state_index = current_state_indexP(str(current_state), q_values_state_table)
            # choose which action to take (i.e., the next job (based on the initial_sol), in which position should be inserted next.)
            picked_action = get_next_actionP(state_index, q_values, epsilon, n_jobs, current_state)
            new_state = current_state
            next_job = Initial_Sol[len(current_state)]
            new_state.insert(picked_action, next_job)  # update the current state
            best_Q_next_state = get_best_Q_next_stateP(str(new_state), q_values_state_table, q_values)
            reward = 1 / objective_value_rewardP(current_state)
            q_values = update_q_valueP(q_values, reward, learning_rate, discount_factor, state_index, picked_action, best_Q_next_state)
            current_state = new_state

        current_state_obj = objective_valueP(current_state)
        if current_state_obj < best_sol_obj:
            best_sol_obj = current_state_obj
            #best_sol_prm = current_state

    print(best_sol_obj, (time.time() - start_time))
